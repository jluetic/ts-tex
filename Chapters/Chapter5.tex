...
t{Sequential recombination algorithms} at hadron colliders two distances which are longitudinally invariant are introduced: $d_{ij}$ which is the distance between each pair of particles and $d_iB$ which is the particle-beam distance. These distances are defined as: 
\begin{equation}
...
\begin{figure}[htbp]
	\centering
		\includegraphics[width=0.8\textwidth]{Figures/diff_algos.pdf}
		%\rule{35em}{0.5pt}
...
\end{equation}     
where offset correction $C_{offset}$ and calibration factors $C_{rel}$ and $C_{abs}$ are applied to both data and simulation, $C_{res}$ is applied only to data. Corrections are applied sequentially, in a fixed order such that $p_T'$ is transverse momentum after applying the offset correction, and $p_T''$ is transverse momentum after applying both offset correction and calibration factors. Correction factors used in this analysis are summarized in \cite{CMS-DP-2013-033}.
